  	...
  	$("#btnAdd").on("click",function(){
  		if(index < 8){
	  		$(this).before('<div id="entry'+index+'" class="clonedInput">'
								+ '<h2 id="reference" name="reference" class="heading-reference">'+(index+1)+'º Dia</h2><hr>'
								+ '<fieldset>'
								+  '<div class="row">'
								...
								+        '<input class="input_fn form-control" name="price[]" id="price_" step="0.01" min="0" max="999" type="number" placeholder="Preço"><br>'
								+      '</div>'
								+      '</div>'
								+    '</div>'
								+  '</div>'
								...
								+       '<input class="gllpSearchButton btn" value="search" type="button">'
								+		'</div></div><br>'
