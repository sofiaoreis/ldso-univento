...
private
def clear_empty_parameters
	[:promoter, :institution, :category, :date, :local].each do | param |
		params.delete(param) if params[param].empty?
	end         
end

def filter_from_parameters
	...
	eventDates2 = Array.new
      ...
      if promoter != nil
      	@eventDates.each do |eventDate|
      		if eventDate.event.promoter.name.upcase.include?(promoter.upcase)
      ...
      if institution != nil
      	@eventDates.each do |eventDate|
      		if eventDate.event.promoter.institution.upcase.include?(institution.upcase)
      			eventDates2.push(eventDate)
      		end
      	end
      	@eventDates = eventDates2.clone
      ...
      if category != nil
      	@eventDates.each do |eventDate|
      		if eventDate.event.category.name.upcase.include?(category.upcase)
      			eventDates2.push(eventDate)
      		end
      	end
      	@eventDates = eventDates2.clone
